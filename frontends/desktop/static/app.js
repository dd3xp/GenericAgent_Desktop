...
function assistantStructuredText(msg) {
  ...
  return Array.isArray(msg.turn_segs) ? msg.turn_segs.join('\n') : '';
}
...
function assistantTurnSegs(msg) {
  if (Array.isArray(msg?.turn_segs)) return msg.turn_segs;
...
function upsert(sess, raw, partial) {
  ...
  if (partial && m.role === 'assistant') {
    ...
    r.draftSegs = Array.isArray(m.turn_segs) ? m.turn_segs : [];
  ...
  try {
    do {
      try {
        ...
        if (busy) await new Promise(z => setTimeout(z, 1000));
        else {
